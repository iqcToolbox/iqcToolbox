...
function this_mult = set.basis_realization(this_mult, basis_realization)
    ...
    if ~isempty(basis_realization)
        ...
        block22 =  ss(kron(basis.a, eye(dim_in)),...
                      ...
                      kron([basis.c; zeros(size(basis.a, 2))], eye(dim_in)),... %This may be wrong
                      kron(blkdiag(basis.d, eye(size(basis.a, 1))),...
                           eye(dim_in)),...
                      basis.Ts);
    ...
    for i = 1:total_time
        if i == 1 || this_mult.quad_time_varying
            ...
            if strcmp(this_mult.region_type, 'box')
                ...
                q11 = blkdiag(this_mult.upper_bound ^ 2 * q11_bnd,...
                              this_mult.upper_rate ^ 2 * q11_rte);
            ...
            elseif strcmp(this_mult.region_type, 'ellipse')
                error(['MultiplierSltvRateBndImpl:',...
                       'set.block_realization'],...
